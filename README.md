...
| [docs/front-end-package-boundary.md](docs/front-end-package-boundary.md) | `Status: draft` — the front-end package boundary `split-opendox-two-layer-product` § 3.4 asks openDox to INVENT: the measured `src/opendox/web/` census, the three destination classes and the test each file must pass, the registration mechanism, the code slices, and SIX open questions, **ALL RULED** — Q1–Q5 (2026-09-12, #656 comment 5642758731) and Q6, the carve-manifest RE-DESTINATION form (#656 comment 5648044785, realized at openxFactory #1011 → `880c821c`). Amendment #3 (2026-09-13) re-measures § 1.1 and the § 3 census at the slice arc's own trees, rewrites § 1.2(d) and § 5's S8 row to the RULED reading *"the path constant moves, the file does not"* (#656 comment 5656343213), declares a fifth class-A tail, and adds § 5.0's landing record. § 7 carries the note's amendment record |
