...
| openDox-code | `#23`'s LANDED head `ce0f28bf` — **added at this amendment's own landing** | leg B as it actually merged, four head moves after the row above: tree `4aaa4bb5`, byte-identical to the squash commit's. Read at fix rounds 14 and 16 only — the figures leg B's movement made false, and `views/viewer.js`'s blob |
| openDox-code | `main` `0b4e8bbf` — **added at this amendment's own landing** | `main` with ALL EIGHT slices landed. Read at fix rounds 14, 16 and 17 only — § 1.1's one deliberately-last figure, § 1.2(d)'s un-narrowing as merged, and `views/viewer.js`'s blob |

*(The first eight rows are the trees amendment #3 MEASURED while the arc was
open; the last two were added at its landing and carry only the figures named
beside them. Where a count below says "all seven trees" it means the seven the
instrument was RUN at — these two are not among them, and no figure taken under
those instruments is restated at them. Rows added at fix round 19, on a Copilot
finding that the block presented eight trees as the whole set while § 1.2(d)
and § 7 were by then reading two more.)*
...
- a file **NAMES A BUNDLE VIEW** when it spells `views/<name>.js` as a slash
  ...
  is wider by one file at exactly one of the seven trees that instrument was RUN at, and § 1.2(d)'s third
  table states both.)*
...
spelling the definition names. Counted over `tests/**.py` at all **seven** trees this instrument was RUN at
(the two landed heads the block gained at fix round 19 are not among them),
...
3. **A SNAPSHOT SCHEMA KEY is not a stage name; a RENDERED WORD or a CORPUS PATH
   ...
   **And it was there all along, which is the part worth admitting.** Unlike
   ...
   `c7a216c7`, `#23`'s head `a7b7ac28`, and, re-read at this amendment's own
   landing, leg B's landed head `ce0f28bf` and openDox-code `main` `0b4e8bbf`
   — so 403 lines and the site at `:262` are not SEVEN readings that happen to
   agree, they are one file no slice in the arc has touched, the whole
   eight-slice arc included. *(This sentence stopped at five trees, and said
   "five readings", until fix round 17: the two landed heads were measured at
   fix round 16 and recorded in § 7 without being carried back here — a Copilot
   finding, accurate.)* (S6 made this row clean by moving the `/source/`
...
2. **No file OUTSIDE class B names a route another column declares** (§ 2.2
   ...
   routes, and openxFactory's three lane routes. Today — **2026-09-11, at
   the census tree `a99eba03`**, the day and the tree every figure in the note
   AS LANDED was read at — it fails on **18 sites**:
...
2026-09-15/16, and RULED Q-L1's annotation-first rule holds for the SEVENTH
and LAST time**, read from the merged pull requests themselves: the annotation
...
leg B openDox-code `#23` → `0b4e8bbf68fabfcd65d4f0d80e619c20a013e888` at **2026-09-16T01:23:16Z**. The annotation led leg A by **1 h 21 min 08 s** and leg B by **1 h 32 min 47 s**.
*(This paragraph said "an EIGHTH time" and the S7 paragraph below said "the
seventh" until fix round 18, and they cannot both be true — a Copilot finding on
openDox-spec `#11`, accurate. Both ordinals were minted at the S7 fill by
counting SLICES LANDED, S1 included, where the rule can only be demonstrated by
a slice that HAS an annotation. Counted as the rule defines itself, one holding
per annotated slice in LANDING order — S2, S3, S6, S4, S5, S7, S8 — S8's is the
**SEVENTH of seven** and S7's the sixth, S1 being the one slice correctly exempt
because it creates files and edits no ARRIVED row. Counted the other way that is
in the table, one per annotation→LEG PAIRING (S6, S5 and S8 carry two legs
each): S2 1, S3 2, S6 3–4, S4 5, S5 6–7, S7 8, S8 **9–10 of ten**. The rule
holds under both readings at every pairing — the count changes, the ordering
fact does not — and every clock it rests on is in this table and re-read from
the merged pull requests.)* Leg A landed at exactly the head this note measured, `080dcfcd`, unmoved; leg B did not — it forward-merged and went on being fixed, `a7b7ac28` → `ce0f28bf`, whose tree is byte-identical to the squash commit's (`4aaa4bb5`), and every figure that movement touches is corrected in place below.
...
SIXTH time, here by 71 seconds** *(this read "the seventh time" until fix round
18; the count and its instrument are in the S8 paragraph above)*. Recorded with it, because it is the act that
...
## 6. Open questions for Brett Heap — ALL SIX RULED
...
*(Amended at this amendment's landing (#3, fix round 15): the SECTION had said
since 2026-09-13 that all six are closed, and the HEADING still read *"Open
questions for Brett Heap"* on its own — so a reader scanning headings met a
section advertising six live questions. The landed words are kept, as everywhere
else in this note, and the status is appended to them. § 3.1's census table
keeps `SPLIT — ruled, not yet built (§ 6)` and `? — undecided (§ 6)` unchanged:
those are class labels measured at the census tree `a99eba03`, not a statement
about this section's questions, and § 3.1's own later columns show `SPLIT` at
**0** once S4 discharged all three.)*
...
**Amendment #3** — this revision, begun 2026-09-13 and first complete
2026-09-14, across slice S5's own landing. **That date is this amendment's
AUTHORING SNAPSHOT and not the last day the record changed**, and it cannot
be: amendment #3 is the revision that fills § 5.0's own landing rows, so it
stays open until the slices it records have merged. § 7's rows below
therefore carry later dates by construction — fix rounds 5 to 11 through
2026-09-14 and 2026-09-15, fix round 12 for S7's landing on 2026-09-15, and
the rows after it for this amendment's own. Each row states the date and the
head it was written at; the header's `Amended:` line keeps 2026-09-13, the
day the revision began, on the precedent of amendments #1 and #2. Claim:
...
| (e) | § 5 (**new § 5.0**) | **the landing record for all eight slices**, with merge shas for the SIX that are merged — S1–S6, S5 completing as this amendment was written (`#1023` → `ee251d6c` 21:44Z, `#18` → `c1ad341a` 21:50Z, `#20` → `8efb3cf5` 23:01Z, in that forced order) — placeholders for S7 and S8, which land ahead of this amendment, and the TWO acts the arc registers and does not perform: the `retired:` row form as the next carve-floor act, and the retirement of the three intent-feed suites under it (RULED `5656343213`). *(That is the row AS AUTHORED, 2026-09-14. The mechanism it describes then worked as written: S7's placeholder cell was filled IN PLACE at S7's own landing on 2026-09-15 — the fix-round-12 row below — and S8's is filled at this amendment's own landing, which is why § 7's rows run past this row's date and why the header paragraph above calls 2026-09-14 the authoring snapshot.)* |
...
| **S8 LANDED** (fix round 14) | § 5.0's S8 row and the paragraph under the table · § 1.1 (the landed reading, the delta sentence, the bundle table's `#23` row) · § 1.2(d)'s un-narrowing figures · the measurement-heads block's `#23` row · this table's fix-round-12 row | **the last placeholder fill, and THREE findings of this amendment's own taken with it.** Slice S8 merged while this amendment was open, in three acts, every value read from the merged pull request itself: the Q-L1 annotation openxFactory `#1025` → `e5896455ac74e126b7baa2ba81cf1cc9ae4d37f1` at **2026-09-15T23:50:29Z**, then leg A openXdox-code `#19` → `0a0265f7e53a1db30a0f51deb556231c2001285f` at **2026-09-16T01:11:37Z**, then leg B openDox-code `#23` → `0b4e8bbf68fabfcd65d4f0d80e619c20a013e888` at **2026-09-16T01:23:16Z**. **RULED Q-L1's annotation-first rule holds an EIGHTH time** *(corrected to the SEVENTH and last at fix round 18)*, and § 5.0's summary goes **SEVEN of the eight → ALL EIGHT**. § 1.1's one deliberately-last figure is filled with it, READ at openDox-code `main` `0b4e8bbf68fabfcd65d4f0d80e619c20a013e888` rather than predicted: 42 blobs / 31,955 lines, 40 hand-authored / 31,953 — `#21`'s head figure UNCHANGED, which is what § 1.1 said it would be. **(1) A finding against § 1.1's own prose, from re-measuring leg B before writing the fill**: *"its whole delta against its base is 7 files under `tests/` (+126 / −9)"* mis-attributes **107 of those 126 lines**. At the head this note measured, SIX of the seven files are under `tests/` (**+19 / −9**) and the SEVENTH is `.github/workflows/validate.yml` (**+107 / −0**) — the UN-NARROWING, the second half of S8's own row title, not a test edit. And neither the count nor the total is a constant, because leg B kept moving after this note read it: measured per file with `git diff --numstat` against openDox-code `main` `1e469713` AS MERGED it is **8 files, +182 / −31** — 7 under `tests/` at **+44 / −27** and the workflow at **+138 / −4**. The claim the sentence exists to make — *not one byte under `src/opendox/web/`* — is untouched and is what the corrected sentence states. The same clause stood in this table's fix-round-12 row and is corrected there too. **(2) `#23` did not land at the head this note measured.** It forward-merged `#21`'s head and `main` and went on being fixed (`a7b7ac28` → `ce0f28bf`, whose tree `4aaa4bb5` is byte-identical to the squash commit's), and at that head `src/opendox/web/` reads **42 / 31,955, 40 hand-authored / 31,953** — the ARC-COMPLETE figure, not the 43-short one. So the 43-line gap § 1.1 measures closed exactly where § 1.1 said it would, from the other side, and the two heads this section separates are one tree by the time they merge. **(3)** The SAME head movement moves one figure and is recorded where the figure is: § 1.2(d)'s un-narrowing reads *"10 files / 283 tests to 27 files / **691** tests"* for `#23`, and 691 is `a7b7ac28`'s — at the landed head the same 27-file list runs **694**. The +3 is slice S7's leg arriving, located exactly rather than asserted: the `validate` file list is BYTE-IDENTICAL at `a7b7ac28`, at the landed head and at the merge (the same 38 references) and a static `def test_` count over it rises **849 → 852** with the whole difference in ONE file, `tests/test_gate_loop_contributed.py` (31 → 34), which reads 852 at openDox-code `main` `1e469713` as well. Both figures are kept, each named with its tree. **And one PREDICTION of this note's was checked at the landed head rather than left standing**: § 3.1's *"one figure in that table moves once more before it lands, and only one"* — `ce0f28bf`'s census fixture is the SAME BLOB as `c7a216c7`'s and as `main` `1e469713`'s (`67c689c8`, against `839a65dc` at `a7b7ac28`), and re-deriving the classes there returns **A 26 / 17,883 · B 1 / 73 · C 14 / 12,564 · `?` 1 / 1,435 = 42 / 31,955**: one figure, once, the named one. Every `a7b7ac28` figure is KEPT throughout, because it is the measurement of the tree amendment #3 read; that head is named as superseded where its identity as `#23`'s HEAD is asserted (the measurement-heads block and § 1.1's bundle table) and where the movement changes a figure (§ 1.2(d)'s un-narrowing, § 3.1's prediction) — everywhere else it is simply a tree a measurement was taken at, which is what it remains. **No other figure, sha or tick moves.** |
| fix round 15 | § 6's heading · the pull-request description | **one review finding, raised as a THREAD, half of it already fixed when the review ran.** Copilot's SEVENTEENTH review, at `5096c27` (review `5217562874`, 2026-09-16T01:30:55Z), reported two inconsistencies. The first is the DESCRIPTION: it said `#1025`, `#23` and `#19` were open and the S8 cell a placeholder. This row recorded that as a snapshot artefact and **it was HALF right** — corrected at fix round 16, which is also where the thread was actually answered and resolved. The LANDING RECORD had indeed been reconciled at **01:27:47Z**, three minutes and eight seconds before the review: § 5 of the description carries all three S8 merge shas and the `0b4e8bbf` bundle figure. But the description's OPENING PARAGRAPH still read *"it lands after openDox-code #23, and it fills its own `[FILL AT LANDING]` placeholders at that landing"* — the future tense the finding names, in the first sentence a reader meets — and still read so at that sitting's last edit of it (`lastEditedAt` **01:32:42Z**, after the review). A review that is right about a document's first paragraph and wrong about its fifth section is not a snapshot artefact, and filing it as one was this record's error and not the review's. *(Round 13 at S7's landing WAS such an artefact, by two minutes, and stays recorded as one.)* The second finding is real and is taken: **§ 6's heading read *"Open questions for Brett Heap"*** while the section has said since 2026-09-13 that all six are RULED and closed, so a reader scanning headings met a section advertising six live questions. The landed words are kept and the status is appended — **"— ALL SIX RULED"** — with a dated note under the section's own closure paragraph. § 3.1's census table is deliberately NOT touched with it: its `SPLIT — ruled, not yet built (§ 6)` and `? — undecided (§ 6)` are CLASS LABELS measured at the census tree `a99eba03`, not claims about this section, and § 3.1's own later columns already show `SPLIT` at **0** once S4 discharged all three. **No figure, sha or tick moves.** |
| fix round 16 | the pull-request description (five places) · this table's fix-round-15 row | **Copilot round 16's thread ANSWERED and RESOLVED, half of its finding UPHELD against this record's own dismissal of it, and a second stale statement of the description's own found while checking the first.** (1) The description's opening paragraph still said the amendment *"lands after openDox-code #23, and it fills its own `[FILL AT LANDING]` placeholders at that landing"* — `#23` merged at `0b4e8bbf` on **2026-09-16T01:23:16Z** and all six markers were filled at `5096c27`, which § 5 of that same description already said. Reconciled, with what it read kept. (2) Found while checking (1), and NOT raised by any review: the description's § 3 still read *"its whole delta is 7 files under `tests/` (+126 / −9)"* — **the exact mis-attribution fix round 14 corrected in § 1.1 of this note**, left standing one screen away in the document that presents it. That is fix round 8's rule arriving a second time: a fix round that corrects a claim in one place and not the other has corrected nothing. The description now carries both readings — six `tests/` files at +19 / −9 plus `.github/workflows/validate.yml` at +107 / −0 at the head it read, and **8 files, +182 / −31** as merged (`git diff --numstat 1e469713 0b4e8bbf`: seven under `tests/` at +44 / −27, the workflow at +138 / −4) — and the claim that holds at every head, `src/opendox/web/` empty in all of them. (3) The description's two measurement tables now name the landed trees the way this note does: § 1.1 gains openDox-code `main` `0b4e8bbf` at **42 / 31,955, 40 / 31,953** and marks `#23`'s head `a7b7ac28` superseded, § 3.1's `a7b7ac28` column likewise, with the census fixture's blob identity as the reason — `67c689c8` at `c7a216c7`, `1e469713`, `ce0f28bf` and `0b4e8bbf`, against `839a65dc` at `a7b7ac28`; and leg A's merge `0a0265f7` is named beside its unmoved head `080dcfcd`. **CHECKED and HELD while there**, because the arc's last obligation hangs on it: `views/viewer.js` is the same blob `4bb5637f`, 403 lines, the stage pill at `:262`, at **seven** trees now — `a99eba03`, `05bbde80`, `8efb3cf5`, `c7a216c7`, `a7b7ac28`, leg B's landed head `ce0f28bf` and openDox-code `main` `0b4e8bbf` — so the fifth tail attaches to a file the whole eight-slice arc never touched. Copilot's EIGHTEENTH review, at `a6f81f53` (review `5217598157`, **01:37:59Z**), announced *"Four documentation inconsistencies identified in the final review must be corrected"* and generated **0 new** comments with no `Suppressed comments` block — the seventh such round on this pull request; it is neither taken nor claimed clean, and the two defects above were found by re-reading the description against this note rather than from it. **No figure, sha or tick of this note moves at this round**; the moves are the description's. |
| fix round 17 | § 1.2(d)'s fifth-tail paragraph (the blob-identity sentence) | **one review finding, raised as a THREAD, accurate, and made by fix round 16 itself.** Copilot's NINETEENTH review, at `8da10d2` (review `5223001576`, 2026-09-16T13:02:33Z), reported that the sentence saying `views/viewer.js` is one blob *"at every tree this note has ever read"* still stopped at FIVE heads and still called them *"five readings"*, while § 5.0 names `ce0f28bf` and `0b4e8bbf` as the landed heads and fix round 16 measured the file at both of them. Correct on every count, and it is the round-8 defect one more time: a fact re-measured in the review record and not carried back into the prose that states it. The sentence now names **seven** trees and the claim it makes is stronger for it — the file is untouched by the WHOLE eight-slice arc, not merely by the part of it this note read while the arc was open. **No figure, sha or tick moves**: the blob is the same `4bb5637f` and the site is the same `:262` at all seven. |
| fix round 18 | § 5.0's S8 paragraph (the ordinal, plus the count and its instrument) · § 5.0's S7 paragraph · this table's fix-round-12 and S8-LANDED rows | **one review finding, raised as a THREAD, accurate in the half that matters most — an ARITHMETIC contradiction this record made twice and checked neither time.** Copilot's TWENTIETH review, at `35b80aa` (review `5223069098`, 2026-09-16T13:07:48Z), observed that the landing table carries a Q-L1 annotation for S2, S3, S4, S6, S5, S7 and S8 with S1 explicitly exempt, so S8 cannot be the EIGHTH holding while S7 is the seventh. Correct, and the origin is locatable: both ordinals were minted at the S7 fill (fix round 12) by counting SLICES LANDED — seven of them by then, S1 included — where the rule is demonstrated only by a slice that HAS an annotation. Counted as defined, one holding per ANNOTATED SLICE in landing order (S2, S3, S6, S4, S5, S7, S8), S7's is the **SIXTH** and S8's the **SEVENTH of seven**, the last the arc can produce; counted per annotation→LEG PAIRING, S7's is the eighth and S8's the **ninth and tenth of ten**. Both readings are now in the note, because the two differ wherever a slice has two legs (S6, S5, S8) and a reader who counts rows will get the first while a reader who counts merges will get the second. **What no reading changes is the ORDERING FACT**: at all seven slices and all ten pairings the annotation merged first, every clock re-read from the merged pull requests at this round (`#1002` 15:45:51Z · `#15` 15:59:44Z · `#1001` 18:36:02Z · `#14` 18:36:20Z · `#1009` 19:25:00Z · `oXc#17` 19:47:33Z · `#16` 19:47:45Z · `#1010` 23:17:15Z · `#17` 23:21:37Z · `#1023` 21:44:17Z · `oXc#18` 21:50:12Z · `#20` 23:01:09Z · `#1030` 21:40:25Z · `#21` 21:41:36Z · `#1025` 23:50:29Z · `oXc#19` 01:11:37Z · `#23` 01:23:16Z). The two § 7 rows that recorded the fills keep what they wrote, each marked with the correction, under this table's no-deletion discipline. **No figure, sha or tick moves — two ordinals do.** |
| fix round 19 | the measurement-heads block (two rows + a scoping note) · § 1.2(d)'s two "seven trees" clauses · this table's fix-round-12 row | **two review findings, raised as threads, both accurate, and the second one this record's own damage.** Copilot's TWENTY-FIRST review, at `10e3b3c` (review `5223190457`, 2026-09-16T13:17:28Z). **(1)** The measurement-heads block presented its eight rows as the trees amendment #3 read, while § 1.2(d) and § 7 had by then read two more — leg B's landed head `ce0f28bf` and openDox-code `main` `0b4e8bbf`. That is the round-8 defect for the third time (a fact measured in one place and not carried back to the block that enumerates it), and the block is the LAST place in the note that may carry it, since it exists so a reader can reproduce every figure. Both heads are now rows, marked as added at the landing and carrying only the figures named beside them, and the two "all seven trees" counts are scoped to the trees their instrument was RUN at, so a reader cannot take the new rows as trees those instruments were re-run over. **(2)** Fix round 18's marker on this table's fix-round-12 row was inserted MID-SENTENCE and split it: *"the same clock evidence § 5."* + the marker + *"0 already carries for S2 and S5."* The sentence is restored with the marker at its end. A record that corrects an ordinal and breaks the sentence it corrects has traded one defect for another, and the review caught it in the same round it was made. **No figure, sha or tick moves**: two rows are added to a block of trees and two counts are scoped to the instrument that produced them. |
